CREATE OR REPLACE FUNCTION public.buy_plan(_plan_id uuid, _amount numeric)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT levels INTO rates FROM public.settings WHERE id = 1;
  rates := COALESCE(rates, ARRAY[10,2,1,4]::numeric[]);
  ...
  WHILE code IS NOT NULL AND lvl <= 4 LOOP
    ...
    EXIT WHEN NOT FOUND;
    commission := round(_amount * COALESCE(rates[lvl], 0) / 100.0, 2);
...
REVOKE EXECUTE ON FUNCTION public.buy_plan(uuid, numeric) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.buy_plan(uuid, numeric) TO authenticated;